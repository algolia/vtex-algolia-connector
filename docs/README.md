...
```
    ...
    "instant-search#header": {
      "props": {
        "indexName": "vtex_day_catalog_products",
        "width": "100%"
      },
      "children": [
        "flex-layout.col#algoliaMenuItem",
        "flex-layout.col#algoliaSearchBox"
      ]
    },
    "flex-layout.col#algoliaMenuItem":  {
      "props": {
...
## CSS Handles
There is a file in the docs folder that has CSS that can be dropped into the css folder in your theme.  The file is called algoliapartnerus.alg-int.css.  To use, rename the file to your account, so it'll be [account].alg-int.css.  The file is [here](/docs/algoliapartnerus.alg-int.css)
